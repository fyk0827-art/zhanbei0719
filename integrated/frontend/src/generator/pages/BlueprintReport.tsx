...
export default function BlueprintReport({ chart }: Props) {
  ...
  const prismPre = useMemo(() => {
    if (!activeChart || !isPrismLifeScript) return null;
    return prismPrecompute(activeChart, {
      name: activeChart.birthData.name,
      gender: activeChart.birthData.gender,
    });
  }, [activeChart, isPrismLifeScript]);
  ...
  useEffect(() => {
    ...
    const text =
      reportType === "full" || reportType === "simple"
        ? buildPrismPreviewCopy(
            prismPrecompute(activeChart, {
              name: activeChart.birthData.name,
              gender: activeChart.birthData.gender,
            }),
          )
        : generateBirthReport500(activeChart, i18n.language);
    savePreviewReportText(text, reportType);
    setReportText(text);
  }, [activeChart, hasAiReport, reportType, i18n.language, reportText]);

  /** 切换语言时重新生成预览报告（AI 深度报告不受影响） */
  useEffect(() => {
    if (!activeChart || isAiReportDone(reportType)) return;
    const text =
      reportType === "full" || reportType === "simple"
        ? buildPrismPreviewCopy(
            prismPrecompute(activeChart, {
  ...
  /** 支付完成后只轮询后端任务；离开页面不会中断生成。 */
  useEffect(() => {
    const aiDone = isAiReportDone(reportType)
      || Boolean(reportText && !isPreviewReport500(reportText));
    if (!reportId || !isPaid || aiDone) return;
    let cancelled = false;
    setGenerating(true);
    setGenError(null);
    const poll = async () => {
      try {
        ...
        if (cancelled) return;
        if (status.startedAt) setGenerationStartedAt(status.startedAt);
        if (status.status === "COMPLETE" && status.reportText) {
          setReportText(status.reportText);
        ...
        if (status.status === "FAILED") {
          setGenError("We couldn't finish your report yet. The issue has been recorded. You can leave this page safely, and we'll email you when your report is ready after it is retried.");
          setGenerating(false);
          return;
        }
      } catch (e) {
  ...
  }, [isPaid, reportId, reportType, reportText]);
  ...
  }, [reportId, reportText, activeChart, reportType, hasAiReport]);
  ...
  if (previewText && !hasAiReport) {
    if (isPaid && generating) {
      return <ReportGenerationWait startedAt={generationStartedAt} />;
    }
    if (isPaid && genError) {
      return (
        <div className="min-h-screen flex flex-col items-center justify-center gap-4 px-6 text-center" style={{ background: "#0D1B2A", color: "#9CA3AF" }}>
          ...
          <div className="flex gap-3">
            ...
            <button type="button" className="px-4 py-2 rounded-lg text-sm text-white" style={{ background: PRIMARY }} onClick={() => refreshUnlock()}>Check again</button>
          </div>
        </div>
      );
    }
    if (isPrismLifeScript && prismPre) {
      return (
        <PrismReportPage
          chart={activeChart}
          pre={prismPre}
        ...
        <section className="py-2 px-6 max-w-[414px] mx-auto">
          <div className="rounded-xl px-4 py-3 text-sm" style={{ background: LIGHT, color: PRIMARY }}>
            <div className="flex items-center gap-2">
              ...
              <span>{t("paywallUnlockedBanner", { name: reportMeta.name })}</span>
            </div>
            {orderId && (
              <p className="text-[11px] mt-2 leading-relaxed opacity-80" style={{ color: DARK }}>
                {t("paywallOrderId")}：<span className="font-mono">{orderId}</span>
                {tradeNo ? (
                  <>
                    <br />
                    {t("paywallTradeNo")}：<span className="font-mono">{tradeNo}</span>
                  </>
                ) : null}
                {paidAtLabel ? <><br />{t("paywallPaidAt")}：{paidAtLabel}</> : null}
              </p>
            )}
          </div>
        </section>
